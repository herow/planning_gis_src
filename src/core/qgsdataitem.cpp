...
{
  fmts << "ppt" << "pptx" << "doc" << "docx" <<  "xls" << "xlsx" << "pdf" << "jpg" << "bat";
...
QVector<QgsDataItem*> QgsDirectoryItem::createChildren()
{
  ...
  foreach ( QString name, fileEntries )
  {
	...
	if(index >= 0)
	{
		QString suffix = name.mid(index + 1).toLower();
		if(fmts.indexOf(suffix) != -1)
		{
			QgsDataItem * item = NULL;
			if (suffix.compare("bat") == 0)
			{
				item = new QgsDataItem(QgsDataItem::DB,this,name.mid(0,index),path);
				item->setIcon(QIcon(":/images/themes/default/mActionOpenTable.png"));
			}
			else{
				item = new QgsDataItem(QgsDataItem::File,this,name,path);
				QString filepath = QgsApplication::pkgDataPath() + "/images/icons/" + suffix +".png";
