...
void QgsCustomization::updateDockWidget()
{
	mSettings->beginGroup("Customization/Title");
	int type  = mSettings->value("windowTitle", 1 ).toInt();
	if(type == 1) {
		(QgisApp::instance())->getZhengZhiGuiHua()->hide();
		(QgisApp::instance())->getBrowserDockWidget()->show();
		(QgisApp::instance())->getOverview()->show();
	}
	else{
		(QgisApp::instance())->getBrowserDockWidget()->hide();
		(QgisApp::instance())->getZhengZhiGuiHua()->show();
		(QgisApp::instance())->getOverview()->hide();
	}
	mSettings->endGroup();
}

QString QgsCustomization::getWindowTitle()
{
	mSettings->beginGroup("Customization/Title");
	int type  = mSettings->value("windowTitle", 1 ).toInt();
	QString windowTitle = QString::fromLocal8Bit("信息平台");
	if(type == 2) {
		windowTitle = QString::fromLocal8Bit("整治规划");
	}
...
Qt::DockWidgetArea QgsCustomization::getLayerTreeArea()
{
