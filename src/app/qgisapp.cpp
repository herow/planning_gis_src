...
static void setTitleBarText_( QWidget & qgisApp )
{
  ...
  //}
  QString caption = QgsCustomization::instance()->getWindowTitle();
    ...
    , mSnappingUtils( 0 )
{
  ...
  QgsCustomization::instance()->updateDockWidget();
  mBrowserWidget2->hide();
...
void QgisApp::createActions()
{
  ...
  connect( mActionWinLayer, SIGNAL( triggered() ), this, SLOT( winLayer() ) );
  connect( mActionIntersection, SIGNAL( triggered() ), this, SLOT( intersection() ) );
  ...
  connect( mActionWeiguan, SIGNAL(triggered()), this, SLOT( weiguan() ) );
  ...
  connect( weiguanAction, SIGNAL( triggered() ), this, SLOT( weiguan_db()) );

  //connect( actionMSDB, SIGNAL( triggered() ), this, SLOT( moshi_db()) );
  //connect( actionCLDB, SIGNAL( triggered() ), this, SLOT( celue_db()) );
...
//  overviewFrame->setLayout(myOverviewLayout);
  ...
  mOverviewDock->setAllowedAreas( Qt::RightDockWidgetArea );
...
void QgisApp::initLayerTreeView()
{
  ...
  addDockWidget( QgsCustomization::instance()->getLayerTreeArea(), mLayerTreeDock );
...
void QgisApp::saveWindowState()
{
  ...
  int type = QgsCustomization::instance()->getType();
  settings.setValue( QString("/UI/state%1").arg(type), saveState() );

  // store window geometry
  settings.setValue( QString("/UI/geometry%1").arg(type), saveGeometry() );
...
void QgisApp::restoreWindowState()
{
  ...
  QSettings settings;
  int type = QgsCustomization::instance()->getType();
  if ( !restoreState( settings.value(  QString("/UI/state%1").arg(type), QByteArray::fromRawData(( char * )defaultUIstate, sizeof defaultUIstate ) ).toByteArray() ) )
  {
    QgsDebugMsg( "restore of UI state failed" );
  }

  // restore window geometry
  if ( !restoreGeometry( settings.value(  QString("/UI/geometry%1").arg(type), QByteArray::fromRawData(( char * )defaultUIgeometry, sizeof defaultUIgeometry ) ).toByteArray() ) )
  {
...
void QgisApp::addDelimitedTextLayer()
{
  ...
  dts->setWindowIcon( QIcon( QgsApplication::iconsPath() + "qgis-icon-60x60.png" ) );
  dts->exec();
  delete dts;
...
#endif
	rlp->setWindowIcon( QIcon( QgsApplication::iconsPath() + "qgis-icon-60x60.png" ) );
    rlp->exec();
...
#endif
	vlp->setWindowIcon( QIcon( QgsApplication::iconsPath() + "qgis-icon-60x60.png" ) );
    if ( vlp->exec() )
    {
...
void QgisApp::completeInitialization()
{
  ...
  //mMapCanvas->zoomToFullExtent();
  //addProject(QgsApplication::pkgDataPath()+ QString::fromLocal8Bit("\\data\\pro\\kongxincun.qgs"));
}
...
{
	QStringList agrs;
	agrs<<(QgsApplication::pkgDataPath() + QString::fromLocal8Bit("\\data\\db\\kongxincun.db"))<<QString("-t")<<QString::fromLocal8Bit("案例村庄");
...
void QgisApp::weiguan()
{
	MainWindow2 *dlg = new MainWindow2();
	dlg->show();
}


void QgisApp::weiguan_db()
{
	weiguanxingtai *dlg = new weiguanxingtai;
	dlg->show();
}
void QgisApp::winLayer()
{
	mLayerTreeDock->show();
}

void QgisApp::intersection()
{
	QStringList agrs;
	QProcess::startDetached(QgsApplication::pkgDataPath() + "\\intersection.exe",agrs);
}