...
void QgsBrowserDockWidget::showContextMenu( const QPoint & pt )
{
  ...
  else if (item->type() == QgsDataItem::File || QgsDataItem::DB)
  {
	  menu->addAction( QString::fromLocal8Bit("打开"), this, SLOT( openfile() ) );
  }
...
void QgsBrowserDockWidget::openfile()
{
	...
	if ( item != NULL && item->type() == QgsDataItem::File )
	{
		QDesktopServices::openUrl ( QUrl::fromLocalFile(item->path()));
	}
	else
	{
		if (item->name().compare(QString::fromLocal8Bit("模式库")) == 0)
		{
			QStringList agrs;
			...
			//QMessageBox::information(NULL, "Title", QString::fromLocal8Bit("模式库"), QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
		}
		else if (item->name().compare(QString::fromLocal8Bit("策略库")) == 0)
		{
			QStringList agrs;
			agrs<<(QgsApplication::pkgDataPath() + QString::fromLocal8Bit("\\data\\db\\kongxincun.db"))<<QString("-t")<<QString::fromLocal8Bit("整治对策表");
			QProcess::startDetached(QgsApplication::pkgDataPath() + "\\sqlitebrowser.exe",agrs);
			//QMessageBox::information(NULL, "Title", QString::fromLocal8Bit("策略库"), QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
		}
		else{
			MainWindow2* xt = new MainWindow2(this);
			xt->show();
		}
	}
}
